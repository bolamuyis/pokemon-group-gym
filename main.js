    ...
    constructor(name) {
        ...
        ASQ(name).then(function (done, name) {
            ...
            for (var i = 1; i < productFrames.length - 1; i++) {
                ...
                axios.get('https://www.googleapis.com/customsearch/v1/?key=AIzaSyCSxsj8MeUdY4tXWbmQp9p6UgnUWN2CB8o&q=' + This.name + '%20' + This.data.moves[i - 1].move.name).then((r) => {
...
ASQ(name).then(function (done, name) {
    ...
    done(setTimeout(console.log,10000,Pokemon));


}).then(function (done, Pokemon) {
    
    done(console.log("New Pokemon", Pokemon));
}).then(function (done, Pokemon) {
